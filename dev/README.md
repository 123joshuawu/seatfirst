...
seeded data actually is and why it's safe/synthetic. Safe to run alongside the production
stack (`docker-compose.yml`) on the same machine — see that file's header comment for the
coexistence rules.
...
accepted.** Read `docs/adr/0034-local-dev-real-amc-fetch-session-mode.md` before using this.
...
**Before setting this to `true`, on a machine that also runs the production stack:**

- Docker-level isolation between `docker-compose.yml` and `docker-compose.dev.yml` is
  verified clean (distinct project names, no shared ports/volumes/networks/bind mounts) —
  see `docker-compose.dev.yml`'s header comment for the full audit.
- **Egress identity is not guaranteed distinct.** If production's `tailscale` service
  (`network_mode: host`, exit-node routing) is active on the same host, ADR 0004 states that
  route "applies to the host's kernel routing, which is what all container egress... rides
  through" — this dev stack's live traffic may ride the same route, not a separate one, no
  matter what `EGRESS_IDENTITY_LABEL` says. Verify your actual egress IP before trusting the
...
- **Never run `AMC_LIVE_FETCH=true` at the same time as an active production fetch pass on
  the same machine.** The two stacks have entirely separate Postgres/Valkey, so there is no
  shared one-in-flight semaphore between them — this tooling cannot enforce that exclusion
  for you, it can only warn (the fetch-worker logs a loud reminder on startup when the flag
  is on). Treat it as an operator discipline, not a guarantee.
- Update `AMC_USER_AGENT`/`EGRESS_IDENTITY_LABEL` in `.env.dev` to values you're comfortable
  being real, attributable identifiers sent with real requests (the shipped defaults already
  satisfy `packages/providers/src/amc/identity.ts`'s validator — product name plus contact —
  but double-check they still say what you want before going live).
